...
function Cars() {
    ...
    const toggleFavorite = async (car_id) => {
        ...
        try {
            const isFavorite = favoriteList.some(
                (favorite) => favorite.car.id && favorite.car.id === car_id
            );
            if (isFavorite) {
                ...
                setFavoriteList(
                    favoriteList.filter(
                        (favorite) => favorite.car.id !== car_id
                    )
                );
    ...
    const typesdesc = [
        `${translates.SUV}`,
        `${translates.Sedan}`,
        `${translates.Hatchback}`,
        `${translates.Convertible}`,
        `${translates.Crossover}`,
        `${translates.Minivan}`,
        `${translates.Pickuptrucks}`,
        `${translates.StationWagon}`,
    ];
    ...
    return (
        <div className="d-flex flex-wrap justify-evenly bg-slate-100 w-100  ">
            <div className="col-5 col-md-2 bg-white p-3">
                <div
                    className="d-flex justify-content-center btn btn-primary align-items-center"
                    onClick={Reset}
                >
                    {" "}
                    <span className="fs-5 ">{translates.reset}</span>{" "}
                    <TbFilterSearch size={40} />
                </div>
                <div className="pt-3 d-flex align-items-center">
                    <span className="text-muted  fs-5 px-1">
                        {translates.Make}:
                    </span>
                    ...
                    <div className="w-70">
                        <input
                            ...
                            placeholder={translates.entermake}
                            onChange={handleMake}
                        />
                    </div>
                </div>
                <div className="pt-2  pb-3 d-flex align-items-center">
                    ...
                    <div className="w-70">
                        <input
                            type="text"
                            className="border px-2 py-1 w-100 "
                            placeholder={translates.entermodel}
                ...
                <hr />
                <div className="py-4">
                    <span className="text-muted mt-1 fs-5 d-flex gap-2 align-items-center">
                        {translates.type}
                    </span>
                ...
                <div className="py-2">
                    <span className="text-muted mt-1 fs-5 d-flex gap-2 align-items-center">
                        {translates.Fueltype} <FaGasPump className="icon" />
                    </span>
                ...
                <hr />
                <div className="py-2">
                    <span className="text-muted mt-1 fs-5 d-flex align-items-center gap-2">
                        {translates.Gear} <GiSteeringWheel className="icon" />
                    </span>
                ...
                <div className="py-2">
                    <span className="text-muted mt-1 fs-5 d-flex align-items-center gap-2">
                        {translates.Capacity} <BsPeopleFill className="icon" />
                    </span>
                    ...
                    <div className="type-list">
                        <div className="price-content">
                            <div className="price-item">
                                ...
                                <p id="min-value" className="m-0">
                                    {minPrice} ₪
                                </p>
                            </div>
                            <div className="price-item">
                                <label htmlFor="">{translates.Max}</label>
                                <p id="max-value" className="m-0">
                                    {maxPrice} ₪
                                </p>
                            </div>
                        </div>
            ...
            <div className="col-6 col-md-10  d-flex flex-wrap justify-evenly">
                {loading ? (
                    ...
                    <div className="fs-4 fw-bold  bg-white align-self-start p-5 rounded mt-5">
                        {" "}
                        No Cars have these features{" "}
                    </div>
                )}
            </div>
        </div>
